...
3. 🚨 **模型预测输出的绝对标准化（以序列为王 + 绝对信任 Schema + 防弹隔离）**：
   ...
   - 🚨 **极简强悍的列提取纪律 (必须用 try-except 包裹)**：
     ...
     ```
4. 🚨 **极简合并与算分死纪律**：
   - `merged_df = pd.merge(gt_df, pred_df[['Standard_ID', 'Model_Prob']], on='Standard_ID', how='left')`
   - 对齐并兜底：`merged_df = merged_df.iloc[:len(gt_df)]`，`merged_df['Model_Prob'] = merged_df['Model_Prob'].fillna(0.0)`
   - 🚨 算分防崩兜底：用 `sklearn.metrics` 算 5 大指标。如果算分环节出现任何 Exception（例如真值全为同一个类别导致 AUC 报错），必须在 except 里拦截并给 5 个指标赋值 0.0！
   - 🚨 写入 CSV 死纪律：必须直接切片赋值：`report_df[f"{{model_name}}_Prob"] = merged_df['Model_Prob'].values`。最后存为 CSV。
...
除了根据架构师的意见修复数据 Bug 外，你必须以极其严厉的口吻，向工程师下达以下**不可违背的文件系统规范**：

1. **强制读入路径**：所有的真值文件和待评测的模型预测文件，**一律存放在相对路径的 `data/` 目录下**！代码读取时必须写为 `data/ground_truth.csv` 或 `data/{模型名}.ext`。
2. 🚨 **规范输出路径（极度致命）**：最终生成的评测结果文件（`eval_result.json`、`final_results_with_predictions.csv` 以及 `evaluation_curves.png`）**必须直接保存在当前根目录（`./`）下**！绝对禁止使用 `os.makedirs` 创建任何如 `results/`、`output/` 的文件夹，也绝对禁止写进 `data/` 目录！保存时必须直接写裸文件名，例如 `report_df.to_csv("final_results_with_predictions.csv")`！
