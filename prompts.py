...
[
    {
        "model_name": "提取出的模型名称(如 AMPlify)",
        "env_name": "为该模型建议的虚拟环境名(小写字母和下划线)",
        "repo_url": "提取并清洗后的开源仓库URL(没有请填 \"\")",
        "dependencies": ["包名1", "包名2==1.0.0"],
        "inference_cmd_template": "预测命令模板。必须用 {fasta_path} 代表输入，{output_dir} 代表输出目录。例如: python predict.py -i {fasta_path} -o {output_dir}/predictions.csv",
